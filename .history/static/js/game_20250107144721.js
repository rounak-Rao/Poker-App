document.addEventListener("DOMContentLoaded", () => {
    ...
    socket.on('cards_dealt', function(data) {
        console
        Object.entries(data).forEach(([playerName, cards]) => {
            const playerSeat = document.querySelector(`.seat[data-player="${playerName}"]`);
            if (playerSeat) {
                ...
                cards.forEach(card => {
                    ...
                    img.src = `/static/cards/${card}`; // Path to your card images
                    img.alt = card;
