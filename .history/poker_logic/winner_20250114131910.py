from player import * #poker_logic.
from deck import *
...
class Winner:
    def __init__(self, players, community_cards):
        self.converted_hands = {
            player.name: [Card.new(f"{card.rank}{card[0].suit}") for card in player.hand]
            for player in players
        }
        self.converted_board = [Card.new(f"{card[0].rank}{card[0].suit}") for card in community_cards] #card[0]
        self.results = []
        self.winners = []
...
if __name__ == "__main__":
    p1 = Player(100, 'r', 0, False)
    p2 = Player(100, 'q', 1, False)
    p3 = Player(100, 'a', 2, False)
    deck = Deck()
    p1.hand = [
           deck.deal(),
           deck.deal()
