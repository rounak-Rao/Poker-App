...
def toggle_game():
    if game_manager.game_state == 'waiting':
        if game_manager.start_game():
            ...
            cards_images = game_manager.card_img()
            socketio.emit('cards_dealt', cards_images, broadcast=True))
...
def handle_seat(player_info: dict):
    ...
    position = player_info['seat_index']
    name = player_info['name']
    stack = player_info['stack']
    player = game_manager.create_player(stack, name, position)
    if player:
        # Broadcast the updated seat to all clients
